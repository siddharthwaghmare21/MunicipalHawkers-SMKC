      ...
      <div style={{ paddingLeft: '5mm', paddingRight: '2mm', paddingTop: '2mm', height: '100%', position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column' }}>
        ...
        {hawker.enrollmentNo && (
          <div style={{ position: 'absolute', bottom: '2mm', right: '35mm', zIndex: 10, backgroundColor: 'white', padding: '1px' }}>
            <QRCodeSVG 
              value={`${process.env.NEXT_PUBLIC_APP_URL || (typeof window !== 'undefined' && !window.location.hostname.includes('localhost') ? window.location.origin : 'https://municipal-hawkers-smkc.vercel.app')}/verify/${encodeURIComponent(hawker.enrollmentNo)}`} 
              size={45} 
              level="M" 
              includeMargin={false}
            />
