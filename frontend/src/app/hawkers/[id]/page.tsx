...
export default async function ViewHawkerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5109'}/api/hawkers/${id}`, {
      headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {hawker?.status !== 'Rejected' && (hawker?.activeLicenseId || (hawker?.licenses && hawker.licenses.length > 0)) && (
            <Link
              href={`/licenses/${hawker.activeLicenseId || hawker.licenses[0].id}/renew`}
